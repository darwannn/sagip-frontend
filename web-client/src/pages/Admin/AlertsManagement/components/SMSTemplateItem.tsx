...
const SMSTemplateItem: React.FC<SMSTemplateItemProps> = memo(
  ({ templateData }) => {
    ...
    const [deleteTemplate, { isError: isDeleteError, error }] =
    ...
    const onDeleteItemHandler = (id: string) => {
      toast.promise(deleteTemplate({ id }).unwrap, {
    ...
    return (
      <>
        <div
          className="template-item p-5 h-[150px] bg-gray-100 shadow-sm rounded-md cursor-pointer relative group hover:shadow-md hover:-translate-y-1 transition-all duration-100"
          onClick={onSelectItemHandler}
        >
          <div className="template-title mb-2">
            <h5 className="font-semibold text-gray-700 truncate">
