...
const SMSForm = () => {
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    ...
    if (res) {
      if (res.success) {
        reset();
        if (setTemplate) dispatch(setSelectedTemplate(null));
      }
    }
  };
  ...
  return (
    ...
    >
      {setTemplate && (
        <div className="p-2 bg-blue-200 my-2 rounded border-l-4 border-l-blue-500 flex flex-row justify-between">
          <span className="text-sm truncate">
            Using template:{" "}
            <span className="font-semibold">{setTemplate?.alertTitle}</span>
          </span>
          <button
            type="button"
      ...
      <div className="mb-5">
        ...
        />
        {errors.location && (
          <span className="input-error">Reciepient location is required.</span>
        )}
      </div>
